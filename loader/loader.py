...
def download_ttfnsw_gtfs(
    api_key: Optional[str] = None,
    dest_path: str = "nsw_gtfs.zip",
    url: str = TFNSW_STATIC_URL,
) -> str:
...
class GTFSStatic:
    ...
    @classmethod
    def from_bytes(
        cls, content: bytes, tables: Optional[List[str]] = None
    ) -> "GTFSStatic":
                    ...
                    try:
                        match fname:
                            case "shapes.txt":
                                ...
                                df = pd.read_csv(
                                    f, usecols=usecols, dtype=dtypes, engine="pyarrow"
                                )
                        ...
                        f.seek(0)
                        df = pd.read_csv(
                            f, dtype=str, encoding="latin1", low_memory=False
                        )
    ...
    @classmethod
    def from_dir(
        cls, dir_path: str, tables: Optional[List[str]] = None
    ) -> "GTFSStatic":
        tables = tables or DEFAULT_TABLES
        loaded = {}
        dp = Path(dir_path)
        for name in tables:
            ...
            if found_path:
                try:
                    ...
                    df = pd.read_csv(
                        found_path, dtype=str, encoding="latin1", low_memory=False
                    )
                loaded[name] = cls._normalise_table(name, df)
        return cls(loaded)
    ...
    def _normalise_table(name: str, df: pd.DataFrame) -> pd.DataFrame:
        match name:
            case "stops":
                ...
                df["location_type_enum"] = pd.to_numeric(
                    df["location_type"], errors="coerce"
                ).map(LocationTypeEnum, na_action="ignore")
            case "stop_times":
                df["stop_sequence"] = pd.to_numeric(
                    df.get("stop_sequence"), errors="coerce"
                )
    ...
    def search_stops(self, query: str, limit: int = 50) -> pd.DataFrame:
        if self.stops.empty:
            return pd.DataFrame()
        mask = self.stops.get("stop_name", pd.Series(dtype=str)).str.contains(
    ...
    def stop_times_for_trip(self, trip_id: str) -> pd.DataFrame:
        ...
        if "stop_sequence" in df.columns:
            try:
                df = (
                    df.assign(_seq=pd.to_numeric(df["stop_sequence"], errors="coerce"))
                    .sort_values("_seq")
                    .drop(columns=["_seq"])
                )
            except Exception:
