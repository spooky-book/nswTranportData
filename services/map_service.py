...
def generate_map_all_routes(
    ...
    *,
    ...
    if parallel:
        features = _build_features_parallel(
            route_to_shapes,
            paths_by_shape,
            route_meta,
    ...
    for feat in features:
        ...
        folium.GeoJson(
            feat,
            name=label,
            style_function=lambda _f, _c=color: {
                "weight": 3,
                "opacity": 0.8,
                "color": f"#{_c}" if _c else "#333333",
            },
            tooltip=folium.GeoJsonTooltip(
                fields=[],
                aliases=[],
                labels=False,
                sticky=True,  # use property 'label'
...
def _pack_make_feature(rid_sids):
    ...
    ]  # for some reason this doesnt work because the sids are strings and not numbers, not sure why this works for the trains but not light rail?
    if decimation_step and decimation_step > 1:
        paths = [decimate_path(p, decimation_step) for p in paths]
...
def add_train_stops_to_map(
    gtfs: GTFSStatic, *, m: folium.Map = None, display_platforms=False
) -> folium.Map:
    stops_copy = gtfs.stops.copy()
    stops_with_location_type = stops_copy.dropna(subset=["location_type"])

    centre = [
        stops_with_location_type["stop_lat"].mean(),
        stops_with_location_type["stop_lon"].mean(),
    ]
    if m is None:
    ...
    for row in stops_with_location_type.itertuples(index=False):
        colour = (
            "blue" if row.location_type_enum == LocationTypeEnum.STATION else "green"
        )
        if (
            not display_platforms
            and row.location_type_enum == LocationTypeEnum.PLATFORMSTOP
        ):
            continue
