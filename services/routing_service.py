...
class StopGraph:
        ...
        def from_gtfs(cls, gtfs: GTFSStatic) -> "StopGraph":
                ...
                if not stop_times.empty:
                        ...
                        if missing:
                                raise KeyError(f"stop_times table missing expected columns: {', '.join(sorted(missing))}")
                ...
                if not gtfs.stops.empty:
                        columns = [col for col in ["stop_name", "parent_station", "stop_lat", "stop_lon"] if col in gtfs.stops.columns]
                        if columns:
                                for row in gtfs.stops.dropna(subset=["stop_id"]).drop_duplicates("stop_id").itertuples(index=False):
                                        stop_id = str(getattr(row, "stop_id")).strip()
                                        if not stop_id:
                                                continue
                                        stop_lookup[stop_id] = {
                                                col: _clean_value(getattr(row, col, None))
                                                for col in columns
                                        }
...
def stops_for_display(df: pd.DataFrame) -> List[Dict[str, Optional[str]]]:
        ...
        available = [col for col in ("stop_id", "stop_name", "parent_station") if col in df.columns]
        if not available:
                return []
