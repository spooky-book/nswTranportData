...
def _load_gtfs_once() -> None:
        ...
        zip_path = Path(zip_path_env) if zip_path_env else DEFAULT_ZIP_PATH
        if not zip_path.exists():
                raise FileNotFoundError(
                        f"Could not find GTFS zip at {zip_path}. "
                        "Download it with main.py before launching the web app."
                )
